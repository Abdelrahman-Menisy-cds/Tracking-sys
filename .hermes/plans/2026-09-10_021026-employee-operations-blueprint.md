...
- [x] Continue implementation — Story 3.2 (submit and correct a timesheet) accepted after QA review and hardening merge; 59 focused timesheet tests + 30 Story 3.1 focused = 89 combined, full suite 287 passed, Django check clean with `config.test_settings`, migration 0006 applied/check clean, `git diff --check` clean (commits `bace208`, merge `60da82d`, hardening `2f7c3bf`, `c525d5f`, merge `9c24acf` = main HEAD; 2026-09-13)
- [x] Continue implementation — Story 3.3 (timesheet review decisions: approve/reject/replace/reopen) accepted after QA review; 60 focused tests passed, Django check clean with `config.test_settings`, `git diff --check` clean; review verified authorization, legal transitions, reasons/confirmation/idempotency-key, replacement after reject, HR reopen, locking/version/idempotency, events/audit, read-only decided sheets, post-commit notification isolation, auth/CSRF/throttle (commits `aa1169b`, merge `1ebe6fd` = main HEAD; 2026-09-13)
- [x] Continue implementation — Story 4.1 (scoped review queues / details) accepted after QA review and read-capability hardening merge; manager queues scoped to current active direct reports, HR timesheet reads `can_decide=false`, safe 404 with no leak, queue filtering/pagination/deterministic sorting verified; final QA APPROVED, review tests 42 passed, Django check clean with `config.test_settings`, `git diff --check` clean (commits `a13a299`, merge `5ced7de`, QA fix `3c70842`, merge `8b15913` = main HEAD; 2026-09-13)
- [x] Continue implementation — Story 4.2 (in-app notifications) accepted after QA review; lock retry/dedupe, post-commit creation, scoped recipient/list/detail/read, safe links, and unread failure isolation verified; final QA APPROVED, notification tests 31 passed with `-W error` and no thread warnings, Django check clean with `config.test_settings`, `git diff --check` clean (main HEAD `a4ff7b9`; 2026-09-13)
- [x] Continue implementation — Story 4.3 (scoped CSV reports) delivered on `main` (`f71b234`, merge `ab7cfae`, QA fix `afa8cb6`, merge `8c6d5e1`; 2026-09-14); acceptance recorded 2026-09-14 with the Story 5.1 entry — its QA evidence was not captured at acceptance time and was reconstructed from git history
- [x] Continue implementation — Story 5.1 (health endpoints, versioned errors, OpenAPI schema) accepted after QA review: final QA APPROVED, combined health/schema suites 40 passed, Django check clean with `config.test_settings`, uniform `X-API-Version` + `X-Request-ID` correlation and the 401/403/404/409/422/429 error matrix verified on real endpoints, bounded readiness timeout with no secret leakage, truthful `GET /api/v1/schema` generated from actual serializers/views; `git diff --check` clean (commits `ef4524e`, merge `0c761c1`, QA fix `ec394d3`, merge `4befcb2`, tests `2fd2362`, schema `dd74aac`, merge `c4acf01` = main HEAD; 2026-09-14)
- [x] Record Story 5.3 acceptance (preference persistence with stale-write guard) — QA APPROVED, `accounts/test_preferences.py` 10 passed, accounts+config 87 passed, Django check clean, makemigrations --check clean, locale ar/en, appearance system/light/dark, defaults ar/system, server-acknowledgement-only save, stale-write 409 with no overwrite, account isolation, OpenAPI schema truthfulness; `git diff --check` clean (commit `65e6b1d`, merge `528284f` = main HEAD; 2026-09-14)
- [ ] Continue implementation with Story 5.4 — release verification and operational gates
